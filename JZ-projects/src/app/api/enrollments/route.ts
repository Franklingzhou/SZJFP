...
export async function GET(request: NextRequest) {
  const session = await checkPermission(request, 'enrollments:read');
...
export async function POST(request: NextRequest) {
  const session = await checkPermission(request, 'enrollments:write');
  if (!session) return unauthorizedResponse();
  try {
    ...
    if (!course_id || !student_id) {
      return NextResponse.json({ error: 'course_id和student_id为必填' }, { status: 400 });
    }
    ...
    // A3: 报名课程后，线索状态 signed → training
    if (data && data.student_id) {
      // student_id 对应 lead 的 user_id，查lead
