...
import { mockCourses, mockReviews } from '@/lib/data-service';
import { REVIEW_SOURCE_LABELS } from '@/lib/types';
import type { ReviewSourceRole } from '@/lib/types';
import { Star, EyeOff, LogOut } from 'lucide-react';
...
export default function InstructorProfilePage() {
  ...
  const toggleHidden = async (id: string) => {
    ...
    try { await updateRecord('reviews', id, { hidden: isHiding }); } catch { /* ignore */ }
  };

  // 统计数据
  const myCourses = mockCourses.filter(c => c.instructorId === user?.id);
  const myStudents = myCourses.reduce((acc: number, c: { currentStudents?: number }) => acc + (c.currentStudents || 0), 0);

  // 加载诚信分
  const [creditScore, setCreditScore] = useState(1000);
  useEffect(() => {
    if (user?.id) {
      fetch('/api/users?role=instructor').then(r => r.json()).then(data => {
    ...
    <div className="pb-20 px-4 pt-4">
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm mb-4">
        <div className="flex items-center gap-3">
          <div className="w-14 h-14 rounded-full bg-amber-100 flex items-center justify-center text-amber-700 text-xl font-semibold">{user?.name?.[0]}</div>
          <div>
            <h2 className="text-lg font-semibold text-slate-800">{user?.name}</h2>
            <p className="text-sm text-slate-500">培训讲师</p>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t">
          <div className="text-center">
            <p className="text-lg font-semibold text-slate-700">{myCourses.length}</p>
            <p className="text-xs text-slate-400">课程</p>
          </div>
          <div className="text-center">
            <p className="text-lg font-semibold text-amber-600">{myStudents}</p>
            <p className="text-xs text-slate-400">学员</p>
          </div>
          <div className="text-center">
              ...
              <div key={review.id} className="bg-slate-50 rounded-lg p-3">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-1">
                    <div className="flex">{[1,2,3,4,5].map(s => <Star key={s} className={`h-3 w-3 ${s <= review.rating ? 'text-amber-400 fill-amber-400' : 'text-slate-200'}`} />)}</div>
                    <button onClick={() => toggleHidden(review.id)} className="ml-1 text-slate-300 hover:text-slate-500"><EyeOff className="h-3.5 w-3.5" /></button>
                  </div>
                </div>
      ...
      <button onClick={() => { logout(); router.push('/m/login'); }} className="w-full py-3 rounded-xl text-sm text-red-500 bg-red-50 flex items-center justify-center gap-2">
