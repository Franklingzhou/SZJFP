...
- **测试体系**: 黄金路径报告v2，按`业务逻辑全景图.md` 4条核心流程逐项覆盖，从17步扩展至72步（10条路径全覆盖）
- **根因反思**: v1黄金路径遗漏推荐弹窗实际流程（只看按钮不看弹窗内容），导致上线后才暴露问题
...
- 自检全量 751/751 通过
- **Hotfix**: 订单大厅/订单管理"选择阿姨"弹窗查询条件 `status=idle` 修正为 `status=available`（数据库实际存储值，664个阿姨中442个为available状态，原idle条件导致弹窗始终为空）
