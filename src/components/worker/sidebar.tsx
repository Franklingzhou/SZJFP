...
const navItems = [
  { href: '/worker/resume', label: '我的简历', icon: FileText },
  { href: '/worker/orders', label: '订单大厅', icon: ClipboardList },
  { href: '/worker/applications', label: '投递记录', icon: Send },
  { href: '/worker/courses', label: '我的课程', icon: GraduationCap },
  { href: '/worker/contracts', label: '我的合同', icon: FileSignature },
  { href: '/worker/notifications', label: '消息通知', icon: Bell },
  ...
  return (
    <aside className={cn(
      ...
      <div className="flex items-center justify-between h-14 px-4 border-b border-slate-200">
        {!collapsed && <span className="font-semibold text-sm text-[#1e3a5f]">阿姨工作台</span>}
