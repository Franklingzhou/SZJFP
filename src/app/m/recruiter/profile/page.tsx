...
import { mockReferrals, mockRecruiterLeads, mockReviews } from '@/lib/data-service';
...
export default function RecruiterProfilePage() {
  ...
  useEffect(() => {
    if (user?.id) {
      fetch('/api/users?role=recruiter').then(r => r.json()).then(data => {
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">招生代理</p>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t">
          <div className="text-center">
            <p className="text-lg font-semibold text-slate-700">{enrolledLeads.length}</p>
            <p className="text-xs text-slate-400">已招学员</p>
          </div>
          <div className="text-center">
