...
import { mockWorkers, mockCourses, mockRecruiterLeads } from '@/lib/data-service';
import { Search, BookOpen, Users, CheckCircle, Clock, ChevronRight, Star, Phone } from 'lucide-react';

export default function InstructorFollowPage() {
  ...
  return (
    <div className="pb-20 px-4 pt-4">
      ...
      <div className="flex gap-4 mb-4 border-b border-slate-100">
        <button
          ...
          className={`pb-2 text-sm font-medium border-b-2 transition-colors ${
            activeTab === 'courses' ? 'border-amber-500 text-amber-600' : 'border-transparent text-slate-400'
          }`}
        >课程管理</button>
        <button
      ...
      {activeTab === 'courses' && selectedCourse && (
        <div>
          <button onClick={() => setSelectedCourse(null)} className="text-sm text-amber-600 mb-3 flex items-center gap-1">
            &larr; 返回课程列表
          </button>
          {(() => {
            const course = courses.find(c => c.id === selectedCourse);
            if (!course) return null;
            // 从共享线索数据中查找该课程的学员（intention匹配课程类型或名称）
              ...
              <div>
                <div className="bg-white rounded-xl p-4 shadow-sm mb-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-3 text-sm">
                    ...
                    <div><span className="text-slate-400">开课：</span><span className="text-slate-700">{course.startDate}</span></div>
                    <div><span className="text-slate-400">结课：</span><span className="text-slate-700">{course.endDate}</span></div>
                  </div>
                </div>
      ...
      {activeTab === 'students' && selectedStudent && (
        <div>
          <button onClick={() => setSelectedStudent(null)} className="text-sm text-amber-600 mb-3 flex items-center gap-1">
            &larr; 返回学员列表
          </button>
          {(() => {
