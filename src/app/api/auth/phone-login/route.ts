  ...
  try {
    ...
    if (user) {
      ...
      if (user.review_status === 'pending') {
        // 预注册用户：自动认领账号
        if (user.register_source === 'pre_registered') {
          const claimed = await claimPreRegisteredUser(user.id);
          if (claimed) {
            // 认领成功，继续登录流程
            console.log('[phone-login] User claimed pre-registered account:', user.id);
          } else {
            return NextResponse.json(
              { error: '账号认领失败，请联系管理员', code: 'CLAIM_FAILED' },
              { status: 500 }
            );
          }
        } else {
...
// 认领预注册账号（将pending状态更新为approved）
async function claimPreRegisteredUser(userId: string): Promise<boolean> {
  try {
    const { getSupabaseClient } = await import('@/storage/database/supabase-client');
    const supabase = getSupabaseClient();
    ...
    if (error) {
      console.error('[claimPreRegisteredUser] Update error:', error);
      return false;
    }

    console.log('[phone-login] Pre-registered user claimed:', userId);
    return true;
  } catch (err) {
    console.error('[claimPreRegisteredUser] Error:', err);
    return false;
  }
}
