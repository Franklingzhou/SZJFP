...
import { getDataVisibilitySync, filterSensitiveFields } from '@/lib/data-permissions';

// 敏感字段（非admin返回时会脱敏）
const sensitiveFields = ['id_card', 'id_card_front', 'id_card_back', 'phone'];

// 脱敏函数
function maskSensitiveFields<T extends Record<string, unknown>>(item: T, role: string): T {
  ...
  for (const field of sensitiveFields) {
    if (field in result && result[field]) {
      ...
      if (value.length > 10) {
        (result as Record<string, unknown>)[field] = value.slice(0, 4) + '****' + value.slice(-4);
      } else {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // 敏感字段脱敏
    const maskedData = filteredData.map(item => maskSensitiveFields(item, session.role));
